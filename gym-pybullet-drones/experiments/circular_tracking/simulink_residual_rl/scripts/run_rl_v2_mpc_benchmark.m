...
rlV2Dir = fullfile(rootDir, 'results', 'policies', 'rl_v2');
if ~exist(dataDir, 'dir'), mkdir(dataDir); end
if ~exist(figDir, 'dir'), mkdir(figDir); end
if ~exist(rlDir, 'dir'), mkdir(rlDir); end
if ~exist(rlV2Dir, 'dir'), mkdir(rlV2Dir); end
addpath(scriptsDir);
...
if exist(policyV2Path, 'file') ~= 2
    train_quadrotor_rl_v2_policy(profile);
end
...
save(fullfile(dataDir, 'quadrotor_rl_v2_mpc_benchmark_results.mat'), ...
    'comparison', 'metrics', 'rlV1Weights', 'rlV2Slots');
writetable(metrics, fullfile(dataDir, 'quadrotor_rl_v2_mpc_benchmark_metrics.csv'));
write_benchmark_metrics_markdown(metrics, fullfile(dataDir, 'quadrotor_rl_v2_mpc_benchmark_metrics.md'));
...
fprintf('RL-v2 benchmark finished. Metrics: %s\n', ...
    fullfile(dataDir, 'quadrotor_rl_v2_mpc_benchmark_metrics.csv'));
end
