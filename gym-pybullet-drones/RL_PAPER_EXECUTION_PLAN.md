...
两种 TD3 可使用相同长度的可测状态、误差和历史动作窗口，以便从状态变化中隐式推断未知扰动。Residual TD3 可额外读取 PID motor command，因为它是上述非特权信息的确定函数，也是残差结构本身的一部分。
...
```text
input  = shared observable state/reference/error history
...
```text
input  = shared observable history + PID motor command
...
结构允许的差异只有：Direct 输出完整 motor command；Residual 输出 PID motor residual，并可读取 PID command。
