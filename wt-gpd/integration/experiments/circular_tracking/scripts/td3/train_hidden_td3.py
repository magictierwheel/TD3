...
RESULTS_ROOT = Path("experiments/circular_tracking/results/hidden_disturbance_td3_paper")
...
def _load_inherited_frozen_pid_config() -> dict[str, Any]:
    ...
    path = _repo_root() / DEFAULT_FROZEN_PATH
    payload = json.loads(path.read_text(encoding="utf-8"))
    parameters = payload.get("parameters")
    if (
        payload.get("schema_version") != 4
        or not isinstance(parameters, Mapping)
        or payload.get("pid_payload_hash") != _canonical_json_hash(parameters)
    ):
        raise ValueError("inherited frozen PID contract is invalid")
    required = {"reference_velocity_gain", "pid_xy_p_scale", "pid_xy_d_scale", "pid_target_step_limit"}
    if set(parameters) != required or not all(np.isfinite(float(parameters[key])) for key in required):
        raise ValueError("inherited frozen PID parameters are invalid")
    return payload
...
def build_training_config(*, mode: str, seed: int, total_timesteps: int) -> dict[str, Any]:
    ...
    return {
        ...
        "frozen_pid_config_path": str(DEFAULT_FROZEN_PATH).replace("\\", "/"),
        ...
        "environment_source_sha256": _canonical_sha256(ENV_SOURCE_PATH),
        "interface_schema": {
...
def _validate_output_folder(output_folder: Path) -> Path:
    ...
    allowed_root = (_repo_root() / RESULTS_ROOT).resolve()
    try:
        resolved.relative_to(allowed_root)
    except ValueError as exc:
        raise ValueError(
            f"output folder must be inside the ignored revised-results namespace: {allowed_root}"
        ) from exc
    return resolved
