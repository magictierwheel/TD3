...
EXPORT_DIR = ROOT / "artifacts" / "report_exports" / "rl_v2"
FIG_DIR = ROOT / "evidence" / "30_rl_v2_mpc_imitation" / "figures"
DATA_DIR = ROOT / "evidence" / "30_rl_v2_mpc_imitation"
...
def write_markdown(rows):
    ...
    lines += [
        ...
        "## 5. 图表文件",
        "",
        "- `evidence/30_rl_v2_mpc_imitation/figures/rl_v2_benchmark_trajectory_3d.png`：六控制器三维轨迹对比。",
        "- `evidence/30_rl_v2_mpc_imitation/figures/rl_v2_benchmark_position_error.png`：位置误差时序。",
        "- `evidence/30_rl_v2_mpc_imitation/figures/rl_v2_benchmark_metric_bars.png`：RMS、稳定段 RMS、终端误差。",
        "- `evidence/30_rl_v2_mpc_imitation/figures/rl_v2_benchmark_effort_feasibility.png`：控制努力、倾角、旋翼饱和率。",
    ]
...
def write_docx(rows):
    ...
    out_path = EXPORT_DIR / f"{STEM}.docx"
    doc.save(out_path)
    return out_path


def export_pdf(docx_path):
    ps1 = ROOT / "reports" / "generators" / "export_docx_pdf.ps1"
