...
EXPORT_DIR = ROOT / "artifacts" / "report_exports" / "strategy_comparison"
FIG_DIR = ROOT / "evidence" / "10_controller_comparison" / "figures"
DATA_DIR = ROOT / "evidence" / "10_controller_comparison"
...
def write_markdown(rows):
    ...
    lines.extend(
        [
            ...
            "## 8. 配套图表",
            "",
            "- `evidence/10_controller_comparison/figures/strategy_circle_trajectory_3d.png`：五种策略在三类环境下的圆周轨迹。",
            "- `evidence/10_controller_comparison/figures/strategy_circle_position_error.png`：位置误差随时间变化。",
            "- `evidence/10_controller_comparison/figures/strategy_circle_metric_bars.png`：RMS、稳定段 RMS、终端误差对比。",
            "- `evidence/10_controller_comparison/figures/strategy_circle_effort_altitude.png`：控制努力和最大高度误差对比。",
            "",
            "![五种策略轨迹对比](../../evidence/10_controller_comparison/figures/strategy_circle_trajectory_3d.png)",
            "",
            "![五种策略误差指标](../../evidence/10_controller_comparison/figures/strategy_circle_metric_bars.png)",
        ]
    )
...
def write_docx(rows):
    ...
    out_path = EXPORT_DIR / f"{REPORT_STEM}.docx"
    doc.save(out_path)
    return out_path
