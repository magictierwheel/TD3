...
%   and Chinese-titled figures under evidence/.

rootDir = matlab_simulink_root();
commonDir = fullfile(rootDir, 'common');
modelsDir = fullfile(rootDir, 'models');
evidenceDir = fullfile(rootDir, 'evidence', '20_residual_rl_v1');
figDir = fullfile(evidenceDir, 'figures');
artifactDataDir = fullfile(rootDir, 'artifacts', '20_residual_rl_v1', 'data');
policyDir = fullfile(evidenceDir, 'policy');
...
save(fullfile(artifactDataDir, 'quadrotor_rl_circle_comparison_results.mat'), ...
    'comparison', 'metrics', 'weights');
writetable(metrics, fullfile(evidenceDir, 'quadrotor_rl_circle_comparison_metrics.csv'));
write_rl_metrics_markdown(metrics, fullfile(evidenceDir, 'quadrotor_rl_circle_comparison_metrics.md'));
plot_rl_figures(comparison, modelTypes, modelLabels, figDir);

fprintf('RL comparison finished. Metrics: %s\n', ...
    fullfile(evidenceDir, 'quadrotor_rl_circle_comparison_metrics.csv'));
end
