...
DEFAULT_FROZEN_PATH = FROZEN_PID_PATH
    ...
    for raw_path in tree.stdout.decode("utf-8").splitlines():
        if raw_path.startswith("gym_pybullet_drones/"):
            ...
            and (
                ...
                or raw_path == str(PROTOCOL_PATH.relative_to(_repo_root())).replace("\\", "/")
            )
        ):
            paths.append(raw_path)
...
try:
    ...
    from experiments.circular_tracking.pybullet_td3.studies.pid_residual_td3.code.training.tune_hidden_pid import _snapshot_worker_entry
...
def _snapshot_worker_entry(request_path: str, response_path: str) -> None:
    ...
    if kind == "probe":
        ...
        environment = importlib.import_module(
            "experiments.circular_tracking.pybullet_td3.studies.pid_residual_td3.code.environments.hidden_disturbance_td3_env"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        payload = tune_and_freeze(
            ...
            command=[sys.executable, "-m", "experiments.circular_tracking.pybullet_td3.studies.pid_residual_td3.code.training.tune_hidden_pid", *effective_argv],
