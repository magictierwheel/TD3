...
legacyPolicyPath = fullfile(rootDir, 'results', 'rl_v2', 'quadrotor_rl_v2_policy.mat');
if exist(policyPath, 'file') == 2
    data = load(policyPath, 'bestPolicySlots');
    policySlots = data.bestPolicySlots(:);
elseif exist(legacyPolicyPath, 'file') == 2
    data = load(legacyPolicyPath, 'bestPolicySlots');
    policySlots = data.bestPolicySlots(:);
else
