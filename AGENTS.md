...
在不违反系统、用户和科学安全约束的前提下，所有智能体必须把“尽快获得科学上可信、边界清楚的结果”置于流程性完美之上：只将会破坏实验公平性、造成数据泄漏、导致奖励/终止/统计错误、使用错误源码配置或静默损坏结果的问题视为阻塞项，其他安全加固、异常恢复、兼容性、性能优化和生产级工程问题统一记录并后置。执行过程中不得因 Minor 问题、普通警告或审查者标签自动开启新的修复循环，不得反复进行完整测试或重复审查；只对当前改动执行必要的针对性检测，确认核心科学硬门满足后立即继续推进。问题应集中处理、一次性汇总审查，避免多代理重复检查和无意义的长时间分析；外部计算期间只进行低频状态监控。待主要任务和实验流程完成后，再统一进行一次完整质量审查、全量测试和后续工程加固。本原则优先于本文件中其他流程性审查、测试和工程化建议，但不允许跳过会使当前科学结论失效的必要验证。
